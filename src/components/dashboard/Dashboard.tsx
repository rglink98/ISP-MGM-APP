...
export default function Dashboard({ user }: DashboardProps) {
  ...
  const inventoryValue = inventory.reduce((acc, item) => {
    ...
    const value = Number(item.valuePerUnit) || 0;
  ...
  const getBalanceByPayment = (type: string) => {
    const income = transactions
      .filter(t => t.type === 'income' && t.paymentType === type)
      .reduce((acc, t) => acc + (Number(t.amount) || 0), 0);
    const expense = transactions
      .filter(t => t.type === 'expense' && t.paymentType === type)
      .reduce((acc, t) => acc + (Number(t.amount) || 0), 0);
  ...
  const pieData = Object.keys(expenseByCategory).map(key => ({
    name: key,
    value: Number(expenseByCategory[key]) || 0
  }));
                ...
                return (
                  <div key={i} className="space-y-1.5">
                    <div className="flex justify-between text-xs">
                      ...
                      <strong className="text-text-main">{stock} {item.unit}</strong>
                    </div>
